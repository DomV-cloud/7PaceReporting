...
namespace TimetrackerReportingClient
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine(
                    "Error: Base URL is required. Set it in appsettings.json or pass it as the first argument."
                );
                Environment.Exit(1);
            }
            ...
            {
                Console.WriteLine(
                    "Error: Token is required. Set it in appsettings.json or pass it with -t."
                );
                Environment.Exit(1);
            }

            var (year, month) =
                cmd.Month > 0 && cmd.Year > 0 ? (cmd.Year, cmd.Month) : AskForMonth();
