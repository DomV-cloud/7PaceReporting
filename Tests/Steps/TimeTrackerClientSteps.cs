...
public static partial class Steps
{
    ...
    public static WireMockServer GivenARunningApiServer()
        => WireMockServer.Start();

    public static TimeTrackerClient GivenAConfiguredClient(string baseUrl, string token)
        => new TimeTrackerClient(baseUrl, token);
    ...
    public static void GivenServerReturnsWorkLogs(
        WireMockServer server, List<WorkLog> workLogs, bool allUsers = false)
    {
    ...
    public static void GivenServerReturnsEmptyWorkLogs(
        WireMockServer server, bool allUsers = false)
    ...
    public static void GivenServerReturnsApiError(
        WireMockServer server, int code, string message)
    {
        var body = JsonConvert.SerializeObject(new ApiResponse<List<WorkLog>>
        {
    ...
    public static List<WorkLog> WhenWorkLogsAreFetched(
        TimeTrackerClient client, int year, int month, bool allUsers = false)
    ...
    public static void ThenRequestShouldHaveCorrectQueryParameters(
        WireMockServer server, int year, int month)
    {
        var fromDate = new DateTime(year, month, 1, 0, 0, 0, DateTimeKind.Utc);
        var toDate = fromDate.AddMonths(1);

        var entry = server.LogEntries.First();
    ...
    {
        server.LogEntries.Should().Contain(entry =>
            entry.RequestMessage.Path == "/api/rest/workLogs/all");
    }

    public static void ThenWorkLogsShouldMatchExpected(
        List<WorkLog> actual, List<WorkLog> expected)
    {
        actual
            .Should()
            .BeEquivalentTo(
